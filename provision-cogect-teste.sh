...
CICD_PROJECT='cogect-teste'
DEV_PROJECT='cogect-teste-app-dev' 
STAGE_PROJECT='cogect-teste-app-hom'

function deploy() {
  oc adm $ARG_OC_OPS new-project $DEV_PROJECT   --node-selector="env=cogect" --display-name=$DEV_PROJECT
  oc adm $ARG_OC_OPS new-project $STAGE_PROJECT --node-selector="env=cogect" --display-name=$STAGE_PROJECT
  oc adm $ARG_OC_OPS new-project $CICD_PROJECT  --node-selector="env=cogect" --display-name=$CICD_PROJECT
  ...
  if [ $LOGGEDIN_USER == 'system:admin' ] ; then
    ...
    oc $ARG_OC_OPS annotate --overwrite namespace $DEV_PROJECT   demo=openshift-cd-$PRJ_SUFFIX >/dev/null 2>&1
    oc $ARG_OC_OPS annotate --overwrite namespace $STAGE_PROJECT demo=openshift-cd-$PRJ_SUFFIX >/dev/null 2>&1
    oc $ARG_OC_OPS annotate --overwrite namespace $CICD_PROJECT  demo=openshift-cd-$PRJ_SUFFIX >/dev/null 2>&1

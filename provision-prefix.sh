...
CICD_PROJECT='citinova-devops-tools'
DEV_PROJECT='citinova-devops-app-dev' 
STAGE_PROJECT='citinova-devops-app-hom'

function deploy() { 

  echo "project cicd  $CICD_PROJECT"
  echo "project dev   $DEV_PROJECT"
  echo "project stage $STAGE_PROJECT"

  oc adm $ARG_OC_OPS new-project $DEV_PROJECT   --node-selector="env=citinova" --display-name=$DEV_PROJECT
  oc adm $ARG_OC_OPS new-project $STAGE_PROJECT --node-selector="env=citinova" --display-name=$STAGE_PROJECT
  oc adm $ARG_OC_OPS new-project $CICD_PROJECT  --node-selector="env=citinova" --display-name=$CICD_PROJECT

  sleep 2

  oc $ARG_OC_OPS policy add-role-to-group edit system:serviceaccounts:$CICD_PROJECT -n $DEV_PROJECT
  oc $ARG_OC_OPS policy add-role-to-group edit system:serviceaccounts:$CICD_PROJECT -n $STAGE_PROJECT

  if [ $LOGGEDIN_USER == 'system:admin' ] ; then
    oc $ARG_OC_OPS adm policy add-role-to-user admin $ARG_USERNAME -n $DEV_PROJECT >/dev/null 2>&1
    oc $ARG_OC_OPS adm policy add-role-to-user admin $ARG_USERNAME -n $STAGE_PROJECT >/dev/null 2>&1
    oc $ARG_OC_OPS adm policy add-role-to-user admin $ARG_USERNAME -n $CICD_PROJECT >/dev/null 2>&1
    
    oc $ARG_OC_OPS annotate --overwrite namespace $DEV_PROJECT   demo=openshift-cd-$PRJ_PREFIX >/dev/null 2>&1
    oc $ARG_OC_OPS annotate --overwrite namespace $STAGE_PROJECT demo=openshift-cd-$PRJ_PREFIX >/dev/null 2>&1
    oc $ARG_OC_OPS annotate --overwrite namespace $CICD_PROJECT  demo=openshift-cd-$PRJ_PREFIX >/dev/null 2>&1

    oc $ARG_OC_OPS adm pod-network join-projects --to=$CICD_PROJECT $DEV_PROJECT $STAGE_PROJECT >/dev/null 2>&1
  fi
  ...
  oc $ARG_OC_OPS new-app -f $template -p $DEV_PROJECT=$DEV_PROJECT -p $STAGE_PROJECT=$STAGE_PROJECT -p DEPLOY_CHE=$ARG_DEPLOY_CHE -p EPHEMERAL=$ARG_EPHEMERAL -p ENABLE_QUAY=$ARG_ENABLE_QUAY -p QUAY_USERNAME=$ARG_QUAY_USER -p QUAY_PASSWORD=$ARG_QUAY_PASS -n $CICD_PROJECT 
}

function make_idle() {
  echo_header "Idling Services"
  oc $ARG_OC_OPS idle -n $DEV_PROJECT --all
  oc $ARG_OC_OPS idle -n $STAGE_PROJECT --all
  oc $ARG_OC_OPS idle -n $CICD_PROJECT --all
}
...
function make_unidle() {
  ...
  for project in $DEV_PROJECT $STAGE_PROJECT $CICD_PROJECT
  do
...
case "$ARG_COMMAND" in
    delete)
        echo "Delete demo..."
        oc $ARG_OC_OPS delete project $DEV_PROJECT $STAGE_PROJECT $CICD_PROJECT
        echo

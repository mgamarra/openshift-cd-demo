...
GITHUB_ACCOUNT=${GITHUB_ACCOUNT:-gamarra}
...
function deploy() {
  ...
  oc adm $ARG_OC_OPS new-project $VAR_CICD_PROJECT  --node-selector="env=citinova" --display-name=$VAR_CICD_PROJECT
  ...
  if [ $LOGGEDIN_USER == 'system:admin' ] ; then
    ...
    oc $ARG_OC_OPS adm policy add-role-to-user admin $ARG_USERNAME -n $VAR_CICD_PROJECT >/dev/null 2>&1
    ...
    oc $ARG_OC_OPS annotate --overwrite namespace $VAR_CICD_PROJECT  demo=openshift-cd-$PRJ_SUFFIX >/dev/null 2>&1
  ...
  local template=https://raw.githubusercontent.com/$GITHUB_ACCOUNT/openshift-cd-demo/$GITHUB_REF/cicd-template-2.yaml
  ...
  oc $ARG_OC_OPS new-app -f $template -p DEV_PROJECT=$VAR_DEV_PROJECT -p STAGE_PROJECT=$VAR_STAGE_PROJECT -p DEPLOY_CHE=$ARG_DEPLOY_CHE -p EPHEMERAL=$ARG_EPHEMERAL -p ENABLE_QUAY=$ARG_ENABLE_QUAY -p QUAY_USERNAME=$ARG_QUAY_USER -p QUAY_PASSWORD=$ARG_QUAY_PASS -n $VAR_CICD_PROJECT 
    ...
    oc set resources dc/nexus --requests=cpu=4000m,memory=6Gi --limits=cpu=6,memory=6Gi 
    oc set probe dc/nexus --remove --readiness --liveness  

    oc set resources dc/sonarqube --requests=cpu=4000m,memory=6Gi --limits=cpu=6,memory=6Gi 
    oc set probe dc/sonarqube --remove --readiness --liveness  
}

function make_idle() {
  echo_header "Idling Services"
  oc $ARG_OC_OPS idle -n $VAR_DEV_PROJECT --all
  oc $ARG_OC_OPS idle -n $VAR_STAGE_PROJECT --all
  oc $ARG_OC_OPS idle -n $VAR_CICD_PROJECT --all
...
function make_unidle() {
  ...
  for project in $VAR_DEV_PROJECT $VAR_STAGE_PROJECT $VAR_CICD_PROJECT
...
case "$ARG_COMMAND" in
    delete)
        echo "Delete demo..."
        oc $ARG_OC_OPS delete project $VAR_DEV_PROJECT $VAR_STAGE_PROJECT $VAR_CICD_PROJECT
